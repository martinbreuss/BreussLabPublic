...
def add_600bp(df, reference):
    '''get region upstream and downstream of the position given in the df.
...
def write_primer3_file(df, path_to_output, padding):
    '''using the regions gathered by add_600bp function, it will write a primer3 file that will allow
...
def parse_p3_output(filepath, counterflag=True, pairs=2):
    ...
    the number of pairs equaling pairs (2 by default).'''
...
print(
'''Usage of {0}: python3 {0} input_file_path reference_genome_path output_directory projID

input:
    
data: .csv file; columns labeled 'CHROM', 'POS' and 'PROJ'
...
csv with fetched sequence
primer3 input file
human readable primer3 output
machine readable primer3 output
csv with primer names and sequences.
ISCPR output files (FASTA and BED)
intermediary files from E-script
FINAL_OUTPUT_FILE_projID
'''.format(sys.argv[0]))
...
data.to_csv(output_dir + name + '_601bp_region'+type_string+'.csv', index=False)

write_primer3_file(data, output_dir + name + '_primer3_input'+type_string+'.txt',
                   60)

with open(output_dir + name + '_primer3_input'+type_string+'.txt', 'rb', 0) as input_f, \
open(output_dir + name + '_human_output'+type_string+'.txt', 'wb', 0) as output_f:
    subprocess.run(['primer3_core', '-format_output'], stdin=input_f,
                   stdout=output_f, check=True)

with open(output_dir + name + '_primer3_input'+type_string+'.txt', 'rb', 0) as input_f, \
open(output_dir + name + '_machine_output'+type_string+'.txt', 'wb', 0) as output_f:
    subprocess.run(['primer3_core'], stdin=input_f,
                   stdout=output_f, check=True)

output = parse_p3_output(output_dir + name + '_machine_output'+type_string+'.txt', pairs=pairs)

output.to_csv(output_dir + name + '_primer_list'+type_string+'.txt', index=False, sep='\t',header=False)
...
def split_primer_list():
    #row_split = []
    primers = pd.read_csv(output_dir +name+'_primer_list'+type_string+'.txt', delimiter='\t',names = ['primer','forward','reverse', 'forward_tm', 'reverse_tm'])
    ...
    if x <= batch_size:
        row_split.append(x)
        print(row_split)
        primers.to_csv(output_dir + name+'_'+str(x) + '_primer_list'+type_string+'.txt', index=False, sep='\t',header=False)
    ...
    if x > batch_size:
        for i in split:
            ...
            if i == batch_size:
                ...
                df.to_csv(output_dir + name+'_'+str(i) + '_primer_list'+type_string+'.txt', index=False, sep='\t',header=False)
                ...
                df.to_csv(output_dir + name+'_'+str(x) + '_primer_list'+type_string+'.txt', index=False, sep='\t',header=False)
...
def ispcr():
    ...
    for i in row_split:
        ...
        subprocess.run('isPcr '+ref_path+' '+output_dir+''+name+'_'+str(i)+'_primer_list'+type_string+'.txt stdout -out=bed -minPerfect=1 -minGood=15 -tileSize=11 -stepSize=5 -maxSize=800 >> '+output_dir+''+name+'_'+str(i)+'_isPcr'+type_string+'.bed',shell=True)

        subprocess.run('isPcr '+ref_path+' '+output_dir+''+name+'_'+str(i)+'_primer_list'+type_string+'.txt stdout -out=fa -minPerfect=1 -minGood=15 -tileSize=11 -stepSize=5 -maxSize=800 >> '+output_dir+''+name+'_'+str(i)+'_isPcr'+type_string+'.fa',shell=True)
...
def ispcr_analysis():
    ...
    for f in row_split:
        if int(os.path.getsize(output_dir + name+'_'+str(f)+'_isPcr'+type_string+'.bed')) == 0:

            continue
        ispcr = pd.read_csv(output_dir + name+'_'+str(f)+'_isPcr'+type_string+'.bed',delimiter='\t',names=['chrom','amplicon_start','amplicon_end','primer_name','score','strand'])
        ...
        for entry in fasta_reader(output_dir+""+name+"_"+str(f)+"_isPcr"+type_string+".fa"):
            seq = str(entry.seq)
        ...
        p3_output = pd.read_csv(output_dir + name+'_'+str(f)+'_primer_list'+type_string+'.txt',delimiter='\t',names=['primer_name','forward_primer','reverse_primer','forward_tm', 'reverse_tm'])
        ...
        results.to_csv(output_dir + name+'_'+str(f)+'_p3_ispcr_output'+type_string+'.txt',sep='\t',index=False)
...
def filter_ispcr_analysis():
    ...
    for i in row_split:
        ...
        if int(os.path.getsize(output_dir + name+'_'+str(i)+'_isPcr'+type_string+'.bed')) == 0:
            continue

        df = pd.read_csv(output_dir + name+'_'+str(i)+'_p3_ispcr_output'+type_string+'.txt',delimiter='\t')
        ...
        df_true.to_csv(output_dir + name+'_'+str(i)+'_score_filtered'+type_string+'.txt',index=False,sep='\t')
...
def split_fasta():
    ...
    for f in row_split:
        ...
        df = pd.read_csv(output_dir + name+'_'+str(f)+'_score_filtered'+type_string+'.txt',delimiter='\t')
        ...
        df.to_csv(output_dir + name+'_'+str(f)+'_final_output'+type_string+'.txt',sep='\t',index=False)
...
def standard_present():
    ...
    for f in row_split:
        ...
        df = pd.read_csv(output_dir +name+'_'+str(f)+'_final_output'+type_string+'.txt',delimiter='\t')
        ...
        df.to_csv(output_dir + name+'_'+str(f)+'_standard_check'+type_string+'.txt',sep='\t',index=False)
...
def standard_matches():
    ...
    for f in row_split:
        ...
        df = pd.read_csv(output_dir + name+'_'+str(f)+'_standard_check'+type_string+'.txt',delimiter='\t')
        good_match = []
        amplicons = []
        percent = []
        match_good_match = []
        ...
        df2.to_csv(output_dir + name+'_'+str(f)+'_good_match_and_amplicons'+type_string+'.txt', sep='\t', index=False)
...
def percent_match():
    ...
    for f in row_split:
        ...
        df2 = pd.read_csv(output_dir + name+'_'+str(f)+'_good_match_and_amplicons'+type_string+'.txt',delimiter='\t')
        ...
        df.to_csv(output_dir + name+'_'+str(f)+'_final_output_with_normalized_match'+type_string+'.txt',sep='\t',index=False)
...
primer_list = pd.read_csv(output_dir + name+'_primer_list'+type_string+'.txt',delimiter='\t', names = ['primer_name', 'forward','reverse'])
...
def capture_best_matches():
    for f in row_split:
        ...
        p3i_output = pd.read_csv(output_dir + name+'_'+str(f)+'_final_output_with_normalized_match'+type_string+'.txt',delimiter='\t')
        ...
        primer_list = pd.read_csv(output_dir + name+'_'+str(f)+'_primer_list'+type_string+'.txt',delimiter='\t', names = ['primer_name', 'forward','reverse'])
...
def write_high_con():
    ...
    high_confidence_from_p3i.to_csv(output_dir + name+'_high_confidence_from_p3i.txt',sep='\t',index=False)
write_high_con()

print("Ideal finished proessing in --- %s seconds ---" % (time.time() - ideal_total_time))

def find_failed_primers_1():
    high_con = pd.read_csv(output_dir + name+'_high_confidence_from_p3i.txt',delimiter='\t')
    #high_con[['fam','chrom','pos','type']] = high_con['amplicon'].str.split('_',expand=True)
    high_con['variant_id'] = high_con['amplicon'].str.split('_',expand=False).str[:-1].str.join('_')
    #high_con['variant_id'] = high_con['fam'] + '_' + high_con['chrom'] + '_' + high_con['pos']
    high_con.to_csv(output_dir + 'high_confidence_with_variant_id',index=False)
    ...
    failed_1.to_csv(output_dir + name+'_failed_primers_right.csv',index=False)
...
def run_right_primers():
    ...
    df = pd.read_csv(output_dir + name+'_failed_primers_right.csv')
    add_600bp(df, reference)

    df.to_csv(output_dir + name + '_601bp_region'+type_string+'.csv', index=False)

    write_primer3_file_right(df, output_dir + name + '_primer3_input_right.txt',
                    60)

    with open(output_dir + name + '_primer3_input_right.txt', 'rb', 0) as input_f, \
    open(output_dir + name + '_human_output_right.txt', 'wb', 0) as output_f:
        subprocess.run(['primer3_core', '-format_output'], stdin=input_f,
                    stdout=output_f, check=True)

    with open(output_dir + name + '_primer3_input_right.txt', 'rb', 0) as input_f, \
    open(output_dir + name + '_machine_output_right.txt', 'wb', 0) as output_f:
        subprocess.run(['primer3_core'], stdin=input_f,
                    stdout=output_f, check=True)

    output = parse_p3_output(output_dir + name + '_machine_output_right.txt', pairs=pairs)
...
def find_failed_primers_2():
    ...
    failed_2.to_csv(output_dir + name+'_failed_primers_left.csv',index=False)
...
def run_left_primers():
    ...
    df = pd.read_csv(output_dir + name+'_failed_primers_left.csv')
    add_600bp(df, reference)

    df.to_csv(output_dir + name + '_601bp_region'+type_string+'.csv', index=False)

    write_primer3_file_left(df, output_dir + name + '_primer3_input_left.txt',
                    60)

    with open(output_dir + name + '_primer3_input_left.txt', 'rb', 0) as input_f, \
    open(output_dir + name + '_human_output_left.txt', 'wb', 0) as output_f:
        subprocess.run(['primer3_core', '-format_output'], stdin=input_f,
                    stdout=output_f, check=True)

    with open(output_dir + name + '_primer3_input_left.txt', 'rb', 0) as input_f, \
    open(output_dir + name + '_machine_output_left.txt', 'wb', 0) as output_f:
        subprocess.run(['primer3_core'], stdin=input_f,
                    stdout=output_f, check=True)

    output = parse_p3_output(output_dir + name + '_machine_output_left.txt', pairs=pairs)
...
high_confidence_from_p3i = pd.read_csv(output_dir + name+'_high_confidence_from_p3i.txt', delimiter='\t')

high_confidence_from_p3i = high_confidence_from_p3i.drop_duplicates(subset=['amplicon']).reset_index()

primer_list_ideal = pd.read_csv(output_dir + name+'_primer_list_ideal.txt',delimiter='\t', names = ['primer_name', 'forward','reverse'])

primer_list_right = pd.read_csv(output_dir + name+'_primer_list_right.txt',delimiter='\t', names = ['primer_name', 'forward','reverse'])

primer_list_left = pd.read_csv(output_dir + name+'_primer_list_left.txt',delimiter='\t', names = ['primer_name', 'forward','reverse'])
...
input_list['TAS-opt'] = ideal_primer_2
...
input_list['concerning_off_targets'] = off_target_2

input_list.to_csv(output_dir +'FINAL_OUTPUT_FILE_'+ name+'.txt',sep='\t',index=False)

all_primers = pd.DataFrame()
all_primers['primers'] = all_primer_names
all_primers.to_csv(output_dir + 'all_primers_list.csv',index=False)


subprocess.run('cat '+name+'_*_normalized_match_*.txt > ALL_OFFTARGETS_FOUND_for_'+name+'.txt',shell=True,cwd=output_dir,capture_output=True)
