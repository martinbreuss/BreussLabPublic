...
@authors: Jonathan Pitsch and Martin Breuss
"""
    ...
    with open(path_to_output, 'a') as output:
        ...
        for index, row in df.iterrows():
            ...
            output.write(('SEQUENCE_ID={}\nSEQUENCE_TEMPLATE={}\n' +
                          ...
                          '=\n').format(str(row['FAMILY']) + '_' + str(row['CHROM']) + '_' +
                                       str(row['POS']) + '_ideal',
                                       row['Sequence_for_Primer3'], str(start),
                                       str(pad2)))

            standard_primer = str(row['FAMILY']) + '_' + str(row['CHROM']) + '_' + str(row['POS']) + '_ideal' 
...
def write_primer3_file_right(df, path_to_output, padding):
    ...
    with open(path_to_output, 'a') as output:
        ...
        for index, row in df.iterrows():
            ...
            right_primer = str(row['FAMILY']) + '_' + str(row['CHROM']) + '_' + str(row['POS']) + '_right'
...
def write_primer3_file_left(df, path_to_output, padding):
    ...
    with open(path_to_output, 'a') as output:
        ...
        for index, row in df.iterrows():
            ...
            left_primer = str(row['FAMILY']) + '_' + str(row['CHROM']) + '_' + str(row['POS']) + '_left'
...
input_list['variant_id'] = input_list['FAMILY'].astype(str) + '_' + input_list['CHROM'].astype(str) + '_' + input_list['POS'].astype(str)
